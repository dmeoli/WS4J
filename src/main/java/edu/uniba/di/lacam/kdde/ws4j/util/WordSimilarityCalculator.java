	...
	public double calcRelatednessOfWords(String word1, String word2, RelatednessCalculator rc) {
		...
		for (POS[] POSPair : rc.getPOSPairs()) {
			...
			if (WS4JConfiguration.getInstance().useMFS()) {
				Concept synset1 = rc.getDB().getMostFrequentConcept(word1, POSPair[0]);
				Concept synset2 = rc.getDB().getMostFrequentConcept(word2, POSPair[1]);
				maxScore = rc.calcRelatednessOfSynsets(synset1, synset2).getScore();
			} else {
				for (Concept synset1 : rc.getDB().getAllConcepts(word1, POSPair[0])) {
					for (Concept synset2 : rc.getDB().getAllConcepts(word2, POSPair[1])) {
